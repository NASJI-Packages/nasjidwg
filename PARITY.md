...
- A sealed object goes out under its original owner whenever that owner
  ...
  the reference's own tree dictionaries lists (a VISUALSTYLE, a SCALE, a
  DICTIONARYVAR — the reference audits a proxy inside its dictionaries,
  so a plain one is re-homed flat under the root, and a chained kind
  stays out); an XRECORD of another generation whose decode stopped
