...
- A sealed object goes out under its original owner whenever that owner
  ...
  DXF-born section or detail view style (the reference checks the
  entries of those two dictionaries before it unwraps a DXF-format
  proxy); a DICTIONARYVAR is the model's own, `drawing.variables`, and goes
