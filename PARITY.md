...
- A sealed object goes out under its original owner whenever that owner
  is in the file (see "Ownership-preserving passthrough" below); only a
  record whose owner is not written — one hung off a layout, a view or
  a table control, whose handles are minted fresh — is re-homed under
  the named objects dictionary or, for the chained kinds, stays home and
  says so. Two R2010 families (VISUALSTYLE, the AcDbAssoc* framework)
  changed spelling at R2013 and cannot travel natively into an AC1032
  file; the R2010 writer that would carry them does not exist.
