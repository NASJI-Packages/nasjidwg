...
- A sealed object goes out under its original owner whenever that owner
  ...
  stays home says so: a class object of another generation that one of
  the reference's own tree dictionaries lists (a VISUALSTYLE, a SCALE —
  the reference audits a proxy inside its dictionaries, so a plain one
  is re-homed flat under the root, and a chained kind stays out; a
  DICTIONARYVAR is the model's own now, `drawing.variables`, and goes
  native into any generation); an XRECORD of another generation whose decode stopped
  short (re-encoding needs every group); and the AcDbAssoc* framework
  of an R2010 source into AC1032. That last family changed spelling at
  R2013 — measured on the reference's own re-saves of an AC1024 sample:
  the dependency records (ACDBASSOCDEPENDENCY, ACDBASSOCVALUEDEPENDENCY,
  the *DEPENDENCYBODY kinds) are bit-identical, but ACDBASSOCNETWORK
  gains four fields behind a bumped class version, ACDBASSOCVARIABLE and
  ACDBASSOCGEOMDEPENDENCY gain fields and lose their string-stream part,
  and ACDBASSOC2DCONSTRAINTGROUP is restructured — not a version field
  to rewrite, and the R2010 writer that would carry the family as it is
  does not exist. The R2010/R2007 VISUALSTYLE set is the reference's
...
| OBJECTS section write (root dict, layouts, groups, mline styles, image defs, table + multileader styles) | ✅ the table and multileader styles are the drawing's own (`drawing.tableStyles` / `drawing.mleaderStyles`), one record per name; the reference reopens our DXF of its tables sample, multileader sample and A-03 sheet at AUDIT 0 with "Drawing Legend", "SCHEDULE" and "BlockMLeader" listed under their dictionaries |
| Ownership through DXF (`writeDxf(d, { preserveHandles })`) | ✅ the DXF codec carries the same ownership facts as the DWG one. Reader: every entity's and record's `xdict` (the `102 {ACAD_XDICTIONARY 360 h 102 }` fence) and `reactors`, each LAYER/LTYPE/STYLE/BLOCK_RECORD/LAYOUT handle, every dictionary that is not the reader's own sealed with its decoded `entries`, hard-owner flag and cloning code (the extension dictionaries and the unmodeled branches of the tree — ACAD_SCALELIST, ACAD_VISUALSTYLE, AcDbVariableDictionary …), each XRECORD sealed under its owner, a hatch's boundary objects. Writer: every sealed object goes out under its original owner when that owner is written (an entity, a block record — the two spaces through their layouts' `blockHandle` — a table record, a layout, a proxy, a sealed object), re-listed by a sealed dictionary from its entries (an entry whose target is not written is left out, this writer's own draw-order table joins the rest), with the `360` fence on the owner and `{ACAD_REACTORS` for every written target, settled to a fixed point (an owner that stays home strands its chain; a dictionary with nothing to list is dropped quietly; a hard 340/360 into nothing keeps the record home); the tree dictionaries are the source's own sealed ones where they travel, rebuilt plain otherwise. Under `preserveHandles` every entity, table record, block record, layout, object and sealed object keeps its source number and fresh ones are minted above the highest — a space out of handle order gets a SORTENTSTABLE under ACAD_SORTENTS in the reference's spelling. An associative hatch leaves with its boundary links (97/330) and the reactor on each boundary entity, non-associative when they are not here. What stays home, said in `drawing.warnings` by kind: a record sealed as DWG bits alone (no DXF spelling — a FIELD or graph node read from a DWG), and from a post-R2000 source the R2008+ spellings the AC1015 filer refuses (TABLECONTENT, DATALINK, and the AcDbAssoc* family, which the reference respells at R2013: its own R2000 DXF spells the constraint group compactly, and it stops on the 2018 form with "Premature end of object"). Proven on the reference (AUDIT 0, census equal, both handle modes): its DXF of A-01 — `(entget (handent "26EFC"))` shows the MTEXT's 360 → ACAD_FIELD → FIELD (evaluated on open), the "Drawing Title" block's xdict → ACAD_ENHANCEDBLOCK → ACAD_EVALUATION_GRAPH with BLOCKLINEARPARAMETER/BLOCKLINEARGRIP/BLOCKSTRETCHACTION and the purge preventer; Site Grading Plan — INSERT 211 → ACAD_FILTER → SPATIAL → SPATIAL_FILTER, 83 FIELDs; Structural - Metric — three blocks' ACAD_ENHANCEDBLOCK graphs and ACAD_SORTENTS tables (the constraint networks stay home, reported); Data Extraction — AUDIT 0 with DATALINK/TABLECONTENT reported home. A DWG-read drawing's dictionaries and XRECORDs leave the same way (A-01.dwg → DXF: 81 dictionaries + 81 XRECORDs under their owners, AUDIT 0). The whole-library legs: the reference's DXF of all 96 samples through readDxf → writeDxf reopen at AUDIT 0 with the original's census, renumbered and preserved alike |
