...
export const aciToRgb = (index: number): number =>
  ACI_RGB[index >= 1 && index <= 255 ? Math.floor(index) : 7];

/** Nearest ACI index (1..255) for a 0xRRGGBB value. */
export const nearestAci = (rgb: number): number => {
...
export const colorToRgb = (c: Color, fallback = 0xFFFFFF): number => {
  if (c.kind === 'rgb') return c.rgb;
  if (c.kind === 'aci') return aciToRgb(c.index);
  return fallback;
};
